...
const noVideoTimeoutMS = 5000; // Error alert if the video fails to connec
...
pubnub = new PubNub ({
	publishKey:'pub-c-94878efd-e172-4149-83b9-686aba75695b',
	subscribeKey : 'sub-c-0c01ccac-630c-11ea-9c0b-5aef0d0da10f'
});
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// UI Render Functions
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
function sendMessage() {
    const messageToSend = messageInput.value.replace(/?<br>|/g, '');
